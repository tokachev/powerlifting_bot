...
def test_dumbbell_single_weight_reps_lines_parse_as_weight_reps() -> None:
    """Dumbbell `9х15` means 9kg × 15 reps, not 9 bodyweight sets."""
    result = parse("жим гантелей стоя\n9х15\n14х15\n14х10х2")
    assert result is not None
    ex = result[0]
    assert len(ex.sets) == 4
    assert [(s.reps, s.weight_kg) for s in ex.sets] == [
        (15, 9),
        (15, 14),
        (10, 14),
        (10, 14),
